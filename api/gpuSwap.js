...
async function gpuSwap(data) {
  let vm1 = await gatherDetailsFromEditVM(
    data.server,
    data.id1,
    undefined,
    data.auth
  );
  let vm2 = await gatherDetailsFromEditVM(
    data.server,
    data.id2,
    undefined,
    data.auth
  ...
  let vm1PrimaryGPU = vm1.edit.pcis.filter(
    (device) => device.gpu && device.checked
  )[0];
  let vm2PrimaryGPU = vm2.edit.pcis.filter(
    (device) => device.gpu && device.checked
  )[0];
  ...
  vm1.edit.pcis.filter(
    (device) => device.id === vm2PrimaryGPU.id
  )[0].bios = Object.assign(
    "",
    vm2.edit.pcis.filter((device) => device.id === vm1PrimaryGPU.id)[0].bios
  );
  vm2.edit.pcis.filter(
    (device) => device.id === vm1PrimaryGPU.id
  )[0].bios = temp;
