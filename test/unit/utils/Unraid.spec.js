...
describe("VM Change Form", () => {
  ...
  const expectedCPUPart =
    "&domain%5Bvcpu%5D%5B%5D=0&domain%5Bvcpu%5D%5B%5D=2&domain%5Bvcpu%5D%5B%5D=4";
  const expectedDiskPart =
    "&disk%5B0%5D%5Bimage%5D=testImage&disk%5B0%5D%5Bselect%5D=testSelect&disk%5B0%5D%5Bsize%5D=testSize&disk%5B0%5D%5Bdriver%5D=testDriver&disk%5B0%5D%5Bbus%5D=testBus";
  const expectedSharePart =
    "&shares%5B0%5D%5Bsource%5D=testSource&shares%5B0%5D%5Btarget%5D=testTarget";
  const expectedPCIPart =
    "&pci%5B%5D=testID&gpu%5B0%5D%5Bid%5D=testID2&gpu%5B0%5D%5Bmodel%5D=qxl&gpu%5B0%5D%5Bkeymap%5D=&gpu%5B0%5D%5Bbios%5D=&audio%5B0%5D%5Bid%5D=testID3&pci%5B%5D=testID4%23remove&pci%5B%5D=testID5%23remove&pci%5B%5D=testID6%23remove";
  const expectedUSBPart = "&usb%5B%5D=testID&usb%5B%5D=testID2%23remove";
  const expectedNetworkPart =
    "&nic%5B0%5D%5Bmac%5D=testMac&nic%5B0%5D%5Bnetwork%5D=testNetwork&nic%5B1%5D%5Bmac%5D=testMac2&nic%5B1%5D%5Bnetwork%5D=testNetwork2";

  test("Get Static Part (Edit)", () => {
    expect(getStaticPart(staticInput, "testID", false)).toEqual(
      expectedStaticPart
    );
  });

  test("Get Static Part (Create)", () => {
    expect(getStaticPart(staticInput, "testID", true)).toEqual(
      expectedStaticPartCreate
    );
  });
...
describe("Detail Extraction", () => {
  ...
  test("Extract Value from HTML", () => {
    const inputToExtractFrom =
      'aloadOfDataWithRandom<HTML><tags with="attributes" and="ids">test</tags></HTML>';
    expect(extractValue(inputToExtractFrom, 'with="', '"')).toEqual(
      "attributes"
    );
  ...
  test("Extract Value from HTML by reversing the string", () => {
    const inputToExtractFrom =
      'aloadOfDataWithRandom<HTML><tags with="attributes">test</tags><tags with="values" selected>test</tags></HTML>';
    expect(
      extractReverseValue(
        extractValue(inputToExtractFrom, "<tags", "selected>"),
        '"',
        'with="'
      )
    ).toEqual("values");
